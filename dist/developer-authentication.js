...
import { signDevJwt, verifyDevJwt, buildCookieHeader, clearCookieHeader, parseCookie, DEFAULT_DEV_SECRET, JWT_HEADER, EMAIL_HEADER, USER_HEADER } from "./jwt.js";
 ...
 *    uniformly.
 */
export function developerAuthentication(settings) {
 ...
 */
export async function handleCallback(c, opts) {
 ...
 */
export async function forwardWithHeaders(c, token, next, logger) {
    ...
    try {
        ...
        const email = defaultTo(payload.email, "");
        const sub = defaultTo(payload.sub, "");
...
//# sourceMappingURL=developer-authentication.js.map