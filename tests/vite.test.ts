...
import { signDevJwt, COOKIE_NAME, JWT_HEADER, EMAIL_HEADER } from "../src/jwt.js";
import type { PathPolicy } from "../src/index.js";
...
/**
 * Run the middleware and resolve once it either calls `next` or ends the
 * response.
 */
function invoke(
...
describe("gating unauthenticated requests", () => {
  ...
  it("lets a non-navigation protected fetch through (Worker enforces 401)", async () => {
    // /dashboard is protected but redirect defaults to true; a fetch
    // (no navigate, no text/html) is neither redirected nor 401'd here.
