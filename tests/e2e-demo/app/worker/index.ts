/**
 * Fixture Worker API for the Vite + Cloudflare Access plugin e2e guard.
 *
 * Note what is NOT here:
 *   - No `developerAuthentication()` — the dev-time Access emulation lives
 *     entirely in `cloudflareAccessPlugin()` (see vite.config.ts).
 *   - No `run_worker_first` in wrangler.jsonc — static assets are served
 *     directly by the asset layer; only `/api/*` reaches this Worker.
 *
 * The Worker uses ONLY the production `cloudflareAccess()` middleware, so it
 * is identical to a real deployment. In dev it validates the plugin's HS256
 * token via HMAC (no network); in production it would validate the real
 * Access RS256 token via JWKS.
 *
 * Dev-token verification is fail-closed and gated on `import.meta.env.DEV`
 * (statically `true` under `vite dev`, `false` in a production build).
 */
