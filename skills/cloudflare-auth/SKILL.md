...
### 3. Serve Static Assets Inside the Hono App — Not via Wrangler Alone
...
```ts
// Serve static assets as the final default route in Hono
...
// Final catch-all: serve the SPA shell for all unmatched routes
...
type Env = {
  Bindings: { CLOUDFLARE_TEAM_DOMAIN: string };
...
Incoming request
  ...
  ├── Policy matches authenticate:false?      → next()            ← public path
  ├── GET /_auth/login                        → render login form
...
Remove `run_worker_first` — all requests route to the Worker:
